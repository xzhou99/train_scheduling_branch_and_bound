...
void CDlgComputeSettings::TestOneRun(int g_SearchFlag , int g_CurComputeMethodNo,int ComputeNumber )
{
   ...
   pfile = fopen("D:\\DTAGUI\\TestSpace.dat","a" );
   fprintf(pfile, "%s",title);
   fclose(pfile);

   pfile = fopen("D:\\DTAGUI\\TestTime.dat","a" );
   fprintf(pfile, "%s",title);
   fclose(pfile);

   pfile = fopen("D:\\DTAGUI\\TestConflict.dat","a" );
   fprintf(pfile, "%s",title);
   fclose(pfile);

   pfile = fopen("D:\\DTAGUI\\TestSummary.dat","a" );
   fprintf(pfile, "%s",title);
   fclose(pfile);

   pfile = fopen("D:\\DTAGUI\\TestSolution.dat","a" );
   fprintf(pfile, "%s",title);
   fclose(pfile);

   for(int t = m_MinTrainNum; t <= m_MaxTrainNum; t+=1)
   {
