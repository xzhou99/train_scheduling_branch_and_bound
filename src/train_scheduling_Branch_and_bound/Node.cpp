...
void CNode::LocateCenter()
{
   ...
   SubLaySize = (int)(pow(2,m_TreeLayer));
...
bool CNode::Intersect(CPoint Point)
{
   ...
   double dts;
   dts=pow(pow(m_X-Point.x,2)+pow(m_Y-Point.y,2),0.5);
...
void CNode::GetLevelNodeID()
{
   ...
   if(pNode->m_NodeID != 0)
   {
	 ...
	 m_LevelNodeID += (int)(pow(2,power) * pNode->m_DecisionFlag );
...
void CNode::ASGenerate() //Task
{
   ...
   for(int k =0; k< m_ConflictTrainSize; k++)
   {
	 ...
	 for(j=0;j< g_SectionSize; j++)
	 {
      ...
      for(int s=0; s< m_ConflictTrainSize;s++)
      {
	 if(s!=k)
	 {
